...
### 跨会话记忆: /memory (2026-09-23)

记住你是谁 — 身份与偏好跨会话携带, 新会话自动注入 system prompt。

- **`/memory` 命令**: `add` / `rm` / `clear` / 空参数列出全部。四种类型:
  `user_profile`(身份/偏好, 默认)、`project_knowledge`(项目知识)、
  `error_patterns`(踩过的坑)、`decisions`(已做的决定)。
  例:`/memory add 我是云丘, 偏好中文回复`。
- 记忆存 `~/.zall/memory.jsonl`, 每个新会话把该用户专享的记忆段接入
  system prompt(USER MEMORY); 同一会话内记下的内容下个会话起生效。
- **修复 Windows 上记忆从未落盘的 bug**: `_save()` 的临时文件名由
  `tempfile` 默认目录拼出来, 含 Windows 非法字符
  (`C:\...\Temp` 的反斜杠/冒号) → 每次写入都 `OSError` 静默失败,
  Windows 用户直到现在其实一条记忆都没存住。改用 `mkstemp` 生成合法
  临时名, 原子替换语义不变。

### 上下文窗口「说实话」: 上游窗口即事实 (2026-09-23)

- **窗口大事实来源换成上游 `/models` 的 `context_length`**: 之前内置表
  是"常见模型名的猜测"(新模型一律默认 32K, 旧值如 deepseek-v4-flash
  曾只写 128k), 实际网关可能给 1M ID — footer 显示 "ctx 100% left /
  32k" 而水位压缩按 32k 提前触发, 全是误导。现在 REPL 启动时后台探测
  已配置 provider 的 `/models`, 顺带收集每个模型的 `context_length` 注入
  窗口表; `/model` 每次打开也刷新。自定义 TOML 的 `window_size` 仍是
  最高优先级(用户显式配置 = 逃生阀)。
- **窗口未知不猜**: 探测/配置/内置表都查不到时, footer 状态栏不再伪造
  "32K" 或百分比, 只显示已用 token 数(真实 usage, 与 Kimi 同口径)。
- **小窗口读数修正**: 固定 12K 底座在小窗口(如 8192)下会凭空吞掉整窗、
  永远显示 0%/吃了 100% —— 底座现在取 `min(baseline, window//4)`
  自适应, 小窗口剩余空间不再假性失血。
- **占位 key 不再算"已配置"**: `sk-secret-456` / `yourkey` / `xxx` 这类
  残留假 key 不再让 provider 出现在列表里(以前会混进一个点了才能用的
  Anthropic)。

### /model 列表: 只列你搭得起的 (2026-09-23)

- 每个已配置 provider 并行探 `/models`(3s 超时, 失败静默), live id 直接
  作为该组模型 — 配置里上游已撤的 id 不再出现; 未配置的 provider 只要
  存在任一已配置就折叠成一行 "not configured", 不再刷一长串别家预设。
  全新安装(什么都没配)保持全量预设可选, 避免空列表。
- 当前模型不在 live 里(自定义名/上游已撤)永远补一行可见, 标 ⚠。

### 选择器交互升级 (2026-09-23)

- **打字即过滤**: /model 菜单里直接输入文字, 候选列表实时收窄到匹配项
  (匹配 value/标签/说明, 不分大小写); 过滤到唯一 → Enter 选中。
- **直输模型名**: 过滤无匹配时 Enter 把输入原文交回 — 自定义模型名直接
  切换(先做字符校验, 命令 `/exit` 这类以及非法字符拒绝, 不会把模型名
  污染成垃圾)。
- 降级路径共用同一契约: 无终端/渲染失败时数字直选、唯一匹配、文本回传
  同一条代码路径, 非交互行为不再与 TTY 有隙缝。

### 修复: 等待行整行空白 (2026-09-23)

- 等待 spinner 的帧从盲文字形 (U+28xx) 换成句点序列 — 盲文在
  Consolas/中文终端缺字形, 等待时整行空白, 看起来像卡死不动。
  现在按 ascii/unicode 模式自动切换, 等待有明确的动态反馈。
